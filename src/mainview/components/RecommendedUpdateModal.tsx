...
import { applyPresetUpdates, markRevisionSeen, type PresetUpdate } from "../../shared/recommended-models";
...
export default function RecommendedUpdateModal({
	...
	/** The catalog the new bindings resolve through — today's plus whatever this
	 *  revision added. Saved only when the user accepts. */
	...
	async function answer(accept: boolean) {
		...
		setBusy(true);
		try {
			// The new models have to exist in the catalog before a preset can point
			// at them; declining adds nothing, so nothing is saved there.
			if (accept) await api.request.modelCatalogSave({ catalog });
			...
			const next = accept ? applyPresetUpdates(agents, updates) : markRevisionSeen(agents, updates);
